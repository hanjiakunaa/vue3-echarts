...
const getData = () => {
  rightBottom({ limitNum: 10 })
    .then((res) => {
      console.log("右下", res);
      if (res.success) {
        state.list = res.data.list.splice(0,10);
      } else {
        ElMessage({
          message: res.msg,
          type: "warning",
        ...
        <li class="right_center_item" v-for="(item, i) in state.list" :key="i">
          ...
          <div class="inner_right">
            ...
            <div class="flex">
              ...
              <div class="info">
                <span class="labels">船名：</span>
                <span class="text-content"> {{ item.terminalno }}</span>
              </div>
              <div class="info">
                <span class="labels">航次：</span>
                <span class="text-content zhuyao"> {{ item.gatewayno }}</span>
              </div>
             
              <div class="info">
                <span class="labels">IMO号：</span>
                <span class="text-content warning"> {{ montionFilter(item.alertvalue) }}</span>
              </div>
              <div class="info">
                <span class="labels">实际抵达接收站：</span>
                <span class="text-content warning">   {{ handleAddress(item) }}</span>
              </div>
              <div class="info">
                <span class="labels">预计抵达日期：</span>
                <span class="text-content warning"> {{ item.createtime }}</span>
              </div>
            </div>

            <!-- <div class="flex">
              <div class="info">
                <span class="labels shrink-0">实际抵达接收站：</span>
                <span class="truncate ciyao" style="font-size: 12px; width: 220px" :title="handleAddress(item)">
